...
export default function Planner() {
  ...
  const formatPrice = (n: number) => formatCurrency(n, budgetCurrency);
  ...
  const generate = async () => {
    ...
    setLoading(true);
    setAiError(false); // Added this line
    try {
      const newDays =
      ...
      setAiError(true);
      toast.error(
        "AI service unreachable. Please try again or use the offline template.",
      );
    } finally {
      setLoading(false);
    }
  ...
  const loadMockTemplate = () => {
    setAiError(false);
    const mockPlan: ItineraryResponse = {
    ...
    while (mockPlan.days.length < form.days) {
      mockPlan.days.push({
        day: mockPlan.days.length + 1,
        theme: "relax",
        activities: [
          "Breakfast at hotel",
    ...
    mockPlan.days = mockPlan.days.slice(0, form.days);
    ...
    toast.success("Loaded offline mock template!");
  };
  ...
  // Action-time server detection to avoid fetch errors on load
  ...
  return (
    <>
      <Helmet>
        <title>Trip Planner | TripGenius</title>
        <meta
          name="description"
          content="Generate an AI-powered travel itinerary."
        />
      </Helmet>
      <div className="mx-auto w-full max-w-7xl px-3 py-6 sm:px-4 md:px-6 lg:py-8">
            ...
            <div className="flex items-start gap-2">
              ...
              <div className="flex-1">
                ...
                <p className="text-sm text-amber-800 dark:text-amber-300 mt-1">
                  You're exploring as a guest. Your trips will be cleared when
                  you logout.{" "}
                  <a
                    href="/signup"
        ...
        <div className="space-y-6 animate-in fade-in duration-700">
          <Card className="w-full animate-in slide-in-from-bottom-8 duration-700 ease-out glass-card hover-glow overflow-hidden border-primary/20 backdrop-blur-2xl bg-background/60">
            <div className="bg-gradient-to-r from-primary/20 via-accent/10 to-transparent p-4 sm:p-6 border-b border-primary/10 relative overflow-hidden">
              ...
              <CardHeader className="p-0 relative z-10">
                ...
                <p className="text-sm text-muted-foreground mt-1">
                  Enter trip details, choose dates or trip length, and let our
                  engine generate a stunning day-by-day plan.
                </p>
              </CardHeader>
            </div>
            <CardContent className="p-4 sm:p-6 space-y-4">
              ...
              <div className="grid grid-cols-1 xs:grid-cols-2 gap-2">
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="text-xs text-muted-foreground flex items-center gap-1">
                    <span>Suggested: {formatPrice(suggestedTotal)}</span>
                    <Button
                      size="sm"
                      variant="ghost"
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="text-xs text-muted-foreground">
                    {members > 0 ? (
                      <>Per day: {formatPrice(perPersonPerDay)} per person</>
                    ) : (
              ...
              <Button
                onClick={generate}
                disabled={loading}
                className="w-full h-10 text-sm font-semibold transition-all duration-300 hover:scale-[1.02] hover:shadow-[0_0_20px_rgba(191,255,255,0.4)] bg-gradient-to-r from-primary to-accent border-0 text-white"
              >
                {loading
                  ? "Generating Route..."
              ...
              <div className="rounded-md bg-secondary p-2 text-xs text-muted-foreground">
                {members > 0 ? (
                  ...
                  <>
                    Daily:{" "}
                    <span className="font-semibold text-foreground">
                      {formatPrice(form.budget / Math.max(1, daysCalc))}
                    </span>
                  </>
                )}
              </div>
          ...
          {aiError ? (
            <Card className="border-red-500/50 bg-red-500/5 mt-8 animate-in fade-in duration-500">
              <CardContent className="py-12 text-center flex flex-col items-center">
                <div className="rounded-full bg-red-100 p-3 dark:bg-red-900/30 mb-4">
                  <CloudSun className="h-8 w-8 text-red-600 dark:text-red-400" />
                </div>
                <h3 className="text-xl font-semibold mb-2">
                  AI Service Unavailable
                </h3>
                <p className="text-muted-foreground max-w-md mx-auto mb-6">
                  We couldn't reach the AI trip generation service. This might
                  be due to a network error or missing API keys.
                </p>
            ...
            <div className="py-16 text-center text-muted-foreground animate-in fade-in duration-700">
              ...
              <p className="max-w-md mx-auto text-sm">
                Fill out your trip details above and let our AI craft the
                perfect customized journey for you.
              </p>
            </div>
          ) : (
            <div className="space-y-6 animate-in slide-in-from-bottom-4 duration-500">
              <div className="sticky top-16 z-10 bg-background/95 backdrop-blur py-2 border-b shadow-sm -mx-3 px-3 sm:mx-0 sm:px-0 sm:shadow-none sm:border-none sm:py-0 sm:static sm:transparent">
                <Tabs
                  value={activeTab}
              ...
              <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
                {activeTab === "map" && (
                  <div className="lg:col-span-2 animate-in fade-in duration-300">
                    <TripMap
                      ...
                      destination={form.destination}
                      originCoords={travel?.coords?.origin}
                      destCoords={travel?.coords?.destination}
                      places={places.map((p) => ({
                        name: p.title,
                      ...
                      <CardHeader className="flex flex-row items-start justify-between space-y-0 pb-2">
                        <div>
                          <CardTitle className="flex items-center gap-2 text-base">
                            <MapIcon className="h-5 w-5 text-primary" /> Nearby
                            Places 🗺️
                          </CardTitle>
                ...
                {activeTab === "itinerary" && (
                  <div className="animate-in fade-in duration-300 lg:col-span-2">
                    <Card className="hidden">
                      <CardHeader>
                        <div className="flex items-center justify-between">
                          <CardTitle className="flex items-center gap-2">
                            <Plane className="h-5 w-5 text-primary" /> Suggested
                            Plan
                          </CardTitle>
                          <Button
                            onClick={exportPdf}
                    ...
                    <BookingOptions destination={form.destination} />
                  </div>
                )}
                    ...
                    <Card>
                      <CardHeader className="space-y-3">
                        <div className="flex items-start justify-between gap-2">
                          <div>
                            <CardTitle className="flex items-center gap-2 text-base">
                              <MapIcon className="h-5 w-5 text-primary" /> Route
                              & Modes 🧭
                            </CardTitle>
                            <p className="text-sm text-muted-foreground mt-2">
                      ...
                      <CardContent className={openRoute ? "" : "hidden"}>
                        <div className="aspect-video w-full overflow-hidden rounded border">
                          {travel?.coords ? (
                            (() => {
                              const o = travel.coords.origin;
                              const d = travel.coords.destination;
                              const pad = 0.5;
                              const minLat = Math.min(o.lat, d.lat) - pad;
                              const maxLat = Math.max(o.lat, d.lat) + pad;
                              const minLon = Math.min(o.lon, d.lon) - pad;
                              const maxLon = Math.max(o.lon, d.lon) + pad;
                              ...
                              return (
                                <iframe
                                  ...
                                  src={`https://www.openstreetmap.org/export/embed.html?bbox=${encodeURIComponent(
                                    bbox,
                                  )}&layer=mapnik&marker=${d.lat}%2C${d.lon}`}
                                  loading="lazy"
                                />
                              );
                            })()
                          ) : (
                            <div className="h-full w-full bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-muted-foreground text-xs">
                        ...
                        {travel ? (
                          <div className="mt-3 text-sm text-muted-foreground">
                            Approx distance: {travel.km} km • Selected:{" "}
                            {mode || "-"}
                          </div>
                        ) : (
                        ...
                        {travel?.options?.length ? (
                          <>
                            <div className="mt-2 rounded border p-2 text-xs">
                              {(() => {
                                const opts = travel.options.filter(
                                  (o) => o.available,
                                );
                            ...
                            {(function () {
                              ...
                              >
                                ...
                                <div className="text-muted-foreground text-xs mt-1">
                                  {o.timeHours}h • {formatPrice(o.price)}
                                </div>
                              </button>
                            ))}
                          </div>
                      ...
                      <CardContent className={openTransport ? "" : "hidden"}>
                        {travel?.options?.length ? (
                          <div className="mb-2 flex flex-wrap items-center gap-1 text-xs">
                            ...
                            {(
                              ["all", "cheapest", "fastest", "eco"] as const
                            ).map((f) => (
                              <button
                                key={f}
                      ...
                      <CardHeader className="space-y-3">
                        <div className="flex items-start justify-between gap-2 flex-wrap">
                          <div className="flex-1 min-w-0">
                            <CardTitle className="flex items-center gap-2 text-base">
                              <CalIcon className="h-5 w-5 text-primary" /> Plan
                              & Calendar 📅
                            </CardTitle>
                            <p className="text-sm text-muted-foreground mt-2">
                      ...
                      <CardContent className={openBudget ? "" : "hidden"}>
                        ...
                        <div className="mt-2 rounded-md border p-2 text-xs">
                          <div className="font-semibold text-foreground">
                            Suggested: {formatPrice(suggestedTotal)}
                          </div>
                          ...
                          <div className="mt-1 text-xs">
                            {form.budget >= suggestedTotal ? (
                              <span className="text-green-600">
                                +{formatPrice(form.budget - suggestedTotal)}
                              </span>
                            ) : (
                              <span className="text-red-600">
                                -{formatPrice(suggestedTotal - form.budget)}
                              </span>
                            )}
                            <Button
                              size="sm"
                  ...
                  <div className="animate-in fade-in duration-300">
                    <Card>
                      <CardHeader className="flex flex-row items-start justify-between space-y-0 pb-2">
                        <div>
                          <CardTitle className="flex items-center gap-2 text-base">
                            <Users className="h-5 w-5 text-primary" /> Group
                            Collab 👥
                          </CardTitle>
                          <p className="text-sm text-muted-foreground mt-2">
                  ...
                  <div className="animate-in fade-in duration-300">
                    <Card>
                      <CardHeader className="space-y-2 pb-2">
                        <CardTitle className="flex items-center gap-2 text-base">
                          <Hotel className="h-5 w-5 text-primary" /> Hotel
                          Search 🏨
                        </CardTitle>
                        <p className="text-sm text-muted-foreground">
                        ...
                        {fx.result ? (
                          <div className="rounded-lg border-2 border-primary/30 bg-primary/5 p-4 space-y-2 mt-2">
                            ...
                            <div className="text-sm text-muted-foreground border-t pt-2">
                              ...
                              1 {fx.from} = {fx.rate.toFixed(4)} {fx.to}
                            </div>
                          </div>
                        ) : (
        ...
        {activeTab === "prep" && (
          <div className="animate-in fade-in duration-300">
            <PackingList
              weather={weather ? { temp: weather.daily?.[0]?.tempMax } : null}
              destination={form.destination}
              days={form.days}
            />
          </div>
...
function ShareTrip({
  ...
  const [token] = useState<string>(() => {
    ...
    const t = cryptoRandom(12);
    localStorage.setItem("tg_trip_code", t);
    return t;
  });

  const shareURL = `${window.location.origin}/trip/share/${token}`;

  const copy = async () => {
    // Persist / refresh trip data every time user hits Copy
    try {
      const shareData = {
      ...
      localStorage.setItem(`tg_share_${token}`, JSON.stringify(shareData));
    } catch {
      // localStorage write failure — continue anyway
    }
    try {
      ...
      // Clipboard not available (HTTP context) — show the URL as fallback
  ...
  return (
    <div className="mt-1 space-y-2">
      <div className="flex items-center gap-1">
        <Input readOnly value={shareURL} className="font-mono h-7 text-xs" />
        <Button
          onClick={copy}
          variant="outline"
          size="sm"
      ...
      <p className="text-[10px] text-muted-foreground">
        Anyone with this link can view the itinerary (no login required).
      </p>
    </div>
  );
}
